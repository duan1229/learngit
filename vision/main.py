...
uart = UART(UART.UART1, 115200,8, None, 1, timeout=1000, read_buf_len=4096)#配置uart
lcd.init() # 初始化lcd
lcd.rotation(2)
    ...
    if code: # 如果检测到人脸
        for i in code: # 迭代坐标框
            ...
            if max_score > 80: # 如果最大分数大于85， 可以被认定为同一个人
                a = img.draw_string(i.x(),i.y(), ("%s:""\n""%2.1f" % (names[index], max_score)), color=(0,255,0),scale=2) # 显示人名 与 分数
                ...
                a = img.draw_string(i.x(),i.y(), ("X:""\n""%2.1f" % (max_score)), color=(255,0,0),scale=2) #显示未知 与 分数
